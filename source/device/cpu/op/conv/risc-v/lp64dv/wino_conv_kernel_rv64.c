...
#include "wino_conv_kernel_rv64.h"
...
void wino_sgemm_4x16(float* output, const float* input, const float* kernel, long cin, short stride_save);
void wino_sgemm_4x4(float* output, const float* input, const float* kernel, long cin, short stride_save);
...
// #ifdef __aarch64__
            ...
            for (int s = 0; s < ELEM_SIZE; s++)
            {
                wino_sgemm_4x16(out_buffer + s * 4 * mulitplier, inp_ptr + s * 4 * cin, ker_ptr + s * PER_OUT_CHAN * cin, cin, wino_out_4_tiles);
            }
...
void wino_sgemm_4x4_rv64(const float* ker, const float* inp, float* output, const float* bias, int cin, int cout_start,
...
// #ifdef __aarch64__
            ...
            {
                {
                    wino_sgemm_4x4(out_buffer + s * 4 * mulitplier, inp_ptr + s * 4 * cin, ker_ptr + s * 4 * cin,
                      ...
                      int num_thread, int cpu_affinity)
{
    ...
    for (int n = 0; n < batch; n++)
    {
        ...
        if (nn_out_c != out_c)
        {
            wino_sgemm_4x4_rv64(interleave_buf, trans_input_buf, output, biases_buf, in_c, nn_out_c, out_c, block_h, block_w,
